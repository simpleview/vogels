  ...
  Joi.validate(config, internals.configSchema, { context: context }, function (err, data) {
    ...
    if(self.timestamps) {
      ...
      if(self.createdAt !== false) {
        if (data.schema && data.schema[createdAtParamName]) {
          valids[createdAtParamName] = data.schema[createdAtParamName];
        } else {
      ...
      if(self.updatedAt !== false) {
        if (data.schema && data.schema[updatedAtParamName]) {
          valids[updatedAtParamName] = data.schema[updatedAtParamName];
        } else {
...
Schema.types.uuid = function () {
  return Joi.string().guid().default(nodeUUID.v4, 'v4 uuid');
};

Schema.types.timeUUID = function () {
  return Joi.string().guid().default(nodeUUID.v1, 'v1 uuid');
};
